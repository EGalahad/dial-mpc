...
@dataclass
class OrcaWalkEnvConfig(BaseEnvConfig):
    kp: Union[float, jax.Array] = field(
        default_factory=lambda: jnp.array(
            [
                ...
                200.0,  # left knee
                60.0,
                60.0,  # left ankle pitch, roll
                200.0,
                200.0,
                200.0,  # right hips
                200.0,  # right knee
                60.0,
                60.0,  # right ankle pitch, roll
                200.0,  # torso
                60.0,
                60.0,
                60.0,  # left shoulder, yaw, pitch, roll
                60.0,
                60.0,
                60.0,  # left elbow, yaw, pitch, roll
                60.0,
                60.0,
                60.0,  # right shoulder, yaw, pitch, roll
                60.0,
                60.0,
                60.0,  # right elbow, yaw, pitch, roll
            ]
        )
    )
    kd: Union[float, jax.Array] = field(
        default_factory=lambda: jnp.array(
            [
                5.0,
                5.0,
...
class OrcaWalkEnv(BaseEnv):
    def __init__(self, config: OrcaWalkEnvConfig):
        ...
        self._torso_idx = mujoco.mj_name2id(
            self.sys.mj_model, mujoco.mjtObj.mjOBJ_BODY.value, "waist_yaw_link"
        )
        ...
        self._init_q = self.sys.init_q
        self._init_qd = jnp.zeros(self._nv)
    ...
    def make_system(self, config: OrcaWalkEnvConfig) -> System:
        # model_path = get_model_path("orca_stable", "orca_description_stable.xml")
        model_path = get_model_path(
            "orca_stable", "orca_description_stable_clsnbox.xml"
        )
    ...
    def step(
        ...
        pos = x.pos[self._pelvis_idx - 1]
        reward_pos = -jnp.sum((pos - pos_tar) ** 2)
        # stay upright reward
        vec_tar = jnp.array([0.0, 0.0, 1.0])
        vec = math.rotate(vec_tar, x.rot[self._pelvis_idx - 1])
        ...
        yaw = math.quat_to_euler(x.rot[self._pelvis_idx - 1])[2]
        d_yaw = yaw - yaw_tar
        ...
        vb = global_to_body_velocity(
            xd.vel[self._pelvis_idx - 1], x.rot[self._pelvis_idx - 1]
        )
        ab = global_to_body_velocity(
            xd.ang[self._pelvis_idx - 1] * jnp.pi / 180.0, x.rot[self._pelvis_idx - 1]
        )
        ...
        reward_height = -jnp.sum(
            (x.pos[self._pelvis_idx - 1, 2] - state.info["pos_tar"][2]) ** 2
        )
        # energy reward
        ...
        # breakpoint_if_isnan(reward)
        # use jax.debug.print to output: z_feet, z_feet_tar, vec, yaw, vb, ab, height
...
# Note that fmt cannot be an f-string because f-strings are formatted immediately, whereas for jax.debug.print, we’d like to delay formatting until later.
        ...
        #     "height: {height}"
        # )
        # jax.debug.print(fmt, z_feet=z_feet, z_feet_tar=z_feet_tar, vec=vec, yaw=jnp.array([yaw]), vb=vb, ab=ab, height=x.pos[self._pelvis_idx - 1, 2])
        ...
        done = jnp.dot(math.rotate(up, x.rot[self._pelvis_idx - 1]), up) < 0
        done |= jnp.any(joint_angles < self.joint_range[:, 0])
        done |= jnp.any(joint_angles > self.joint_range[:, 1])
        done |= pipeline_state.x.pos[self._pelvis_idx - 1, 2] < 0.18
...
brax_envs.register_environment("orca_walk", OrcaWalkEnv)
