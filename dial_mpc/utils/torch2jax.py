...
connect(torch.sub, jnp.subtract)
...
connect(torch.clamp_min, partial(jnp.clip, max=None))
...
def broadcast_tensors(*tensors):
  return [Torchish(x) for x in jnp.broadcast_arrays(*[coerce(x) for x in tensors])]
...
def mish(x, inplace=False):
  # Can't use `connect` since jax.nn.mish does not have an `inplace` option.
  if inplace:
    assert isinstance(x, Torchish)
    x.value = jax.nn.mish(x.value)
    return x
  else:
    return jax.nn.mish(coerce(x))
...
@implements(torch.gru_cell)
def gru_cell(input: Torchish, hx: Torchish, w_ih: Torchish, w_hh: Torchish, b_ih: Optional[Torchish] = None, b_hh: Optional[Torchish] = None) -> Torchish:
    ...
    """
    input, hx = coerce(input), coerce(hx)
    w_ih, w_hh = coerce(w_ih), coerce(w_hh)
    b_ih, b_hh = coerce(b_ih), coerce(b_hh)
...
def convert_io_torchish_to_jax(func):
  def wrapper(x: Torchish, num_or_sections, dim=0):
    x = coerce(x)
    outputs = func(x, num_or_sections, dim)
    return [Torchish(y) for y in outputs]
  return wrapper

@implements(torch.split, JAXishify_output=False)
@convert_io_torchish_to_jax
@partial(jax.jit, static_argnames=('num_or_sections', 'dim'))
def split(x: jnp.ndarray, num_or_sections, dim=0) -> list[jnp.ndarray]:
  if isinstance(num_or_sections, int):
    return [x for x in jnp.array_split(x, num_or_sections, axis=dim)]
  else:
    # cumsum num_or_sections
    num_or_sections = [num_or_sections[0],]
    return [x for x in jnp.array_split(x, num_or_sections, axis=dim)]

rnng = jax.random.PRNGKey(0)

def manual_seed(seed):
  global rnng
  rnng = jax.random.PRNGKey(seed)

@implements(torch.normal)
def normal(mean, std, *, generator=None):
  global rnng
  rnng, rnng_normal = jax.random.split(rnng)
  noise = jax.random.normal(rnng_normal, mean.shape, mean.dtype)
...
def t2j_module(module):
  def f(x, state_dict={}):
    ...
    if isinstance(module, TensorDictModule):
      state_dict = {f"module.{key}": val for (key, val) in state_dict.items()}
    breakpoint()
